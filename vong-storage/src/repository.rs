...
pub fn finalize_session(conn: &Connection, session_id: i64) -> Result<(), StorageError> {
    ...
    let updated = conn.execute("UPDATE sessions SET ended_at = ?1, duration_ms = ?1 - started_at WHERE id = ?2", params![now_ms, session_id])?;
    if updated == 0 { return Err(StorageError::SessionNotFound(session_id)); }
    Ok(())
}
...
pub fn fetch_segments_for_session(conn: &Connection, session_id: i64) -> Result<Vec<Segment>, StorageError> {
    ...
    let rows = stmt.query_map([session_id], |row| {
        ...
        Ok(Segment {
            id: row.get(0)?, session_id: row.get(1)?, seq: row.get(2)?, start_ms: row.get(3)?, end_ms: row.get(4)?,
            speaker_label: row.get(5)?, text: row.get(6)?, language: row.get(7)?, confidence: row.get(8)?,
...
pub fn list_recent_sessions(conn: &Connection, limit: u32) -> Result<Vec<Session>, StorageError> {
    ...
    let rows = stmt.query_map([limit], |row| Ok(Session {
        id: row.get(0)?, started_at_ms: row.get(1)?, ended_at_ms: row.get(2)?, duration_ms: row.get(3)?,
        audio_source: row.get(4)?, provider: row.get(5)?, detected_language: row.get(6)?,
        audio_path: row.get(7)?, meta_json: row.get(8)?,
    }))?.collect::<Result<Vec<_>, _>>()?;
...
pub fn delete_session(conn: &Connection, id: i64) -> Result<(), StorageError> {
    ...
    if n == 0 { return Err(StorageError::SessionNotFound(id)); }
    Ok(())
}
