...
pub fn search_transcripts(conn: &Connection, query: &str, limit: u32) -> Result<Vec<SearchHit>, StorageError> {
    let q_norm = normalize(query.trim());
    if q_norm.is_empty() {
        return Err(StorageError::Invalid("query is empty".into()));
    }
    let fts_query = q_norm.split_whitespace().filter(|t| !t.is_empty()).map(escape_fts_token).collect::<Vec<_>>().join(" AND ");
    if fts_query.is_empty() {
        return Err(StorageError::Invalid("query contained only whitespace".into()));
    }
    ...
    let hits = stmt.query_map(params![&fts_query, limit], |row| Ok(SearchHit {
...
mod tests {
    ...
    fn tone_insensitive_search_finds_voiced() {
        let conn = setup_with_texts(&["Cuoc hop team chieu nay rat hay", "Toi di an com toi", "Hop voi khach hang vao thu Hai"]);
        let hits = search_transcripts(&conn, "hop", 10).unwrap();
        assert!(!hits.is_empty(), "search hop should match hop variants via remove_diacritics=2");
